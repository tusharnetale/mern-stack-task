...
async function generateNestedComments(
  ...
  for (let i = 0; i < numberOfReplies; i++) {
    const userId = faker.number.int({ min: 1, max: noOfUsers });
    const comment = {
...
export default async function seedComments(
  ...
  // Generate primary comments for each product
  for (let productId = 1; productId <= noOfProducts; productId++) {
    ...
    for (let i = 0; i < numberOfPrimaryComments; i++) {
      ...
      await generateNestedComments(productId, primaryCommentId, noOfUsers);
    }
  }
}
